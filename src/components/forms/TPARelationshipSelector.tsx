...
export default function TPARelationshipSelector({
  ...
  useEffect(() => {
    ...
    const tpaWithRelationships: BulkPayerMappingItem = {
      payer_id: tpa.auto_id || tpa.id,
      payer_name: tpa.name,
      payer_type: tpa.type,
      affiliated_insurance_companies: selectedInsurances.map(id => {
        const insurance = availableInsuranceCompanies.find(p => p.auto_id === id || p.id === id)
        return {
          payer_id: insurance?.auto_id || insurance?.id || id,
          payer_name: insurance?.name || '',
        }
      }),
    }
    onChange(tpaWithRelationships)
  }, [selectedInsurances, tpa, availableInsuranceCompanies, onChange])
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              <CardDescription className="text-xs mt-1">
                {tpa.code && `Code: ${tpa.code}`}
                {selectedInsurances.length > 0 && (
                  <span className="ml-2 text-blue-600 font-medium">
                    • {selectedInsurances.length} insurance{' '}
                    {selectedInsurances.length === 1 ? 'company' : 'companies'} selected
                  </span>
                )}
              </CardDescription>
            </div>
