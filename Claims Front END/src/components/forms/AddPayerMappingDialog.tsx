  ...
  useEffect(() => {
    const fetchPayers = async () => {
      ...
      try {
        ...
        // Get affiliated payers for the hospital
        const response = await payerAffiliationsApi.getActivePayerAffiliations()
        
        // Convert affiliations to AvailablePayer format
        const payersFromAffiliations: AvailablePayer[] = response.affiliations.map(affiliation => ({
          payer_id: affiliation.payer_id,
          payer_name: affiliation.payer_name,
          payer_code: affiliation.payer_code,
          payer_type: affiliation.payer_type,
          status: affiliation.status,
        }))
        
        setAvailablePayers(payersFromAffiliations)
      } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select one or more affiliated payers to map to this tariff. Only payers that your hospital is affiliated with are shown.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div className="border rounded-lg p-3 max-h-64 overflow-y-auto scrollbar-hide space-y-2">
                      {availablePayers.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                          <p className="font-medium">No affiliated payers found</p>
                          <p className="text-sm mt-1">Please add payer affiliations first from the Payer Affiliations page.</p>
                        </div>
                      ) : (
